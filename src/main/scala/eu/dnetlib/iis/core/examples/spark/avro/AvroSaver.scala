 ...
 */
package eu.dnetlib.iis.core.examples.spark.avro
...
object AvroSaver {
  ...
  def save(dataFrame: DataFrame, avroSchema: Schema, location: String): Unit = {
    implicit lazy val encoder: Encoder[(AvroKey[GenericRecord], NullWritable)] =
    ...
    dataFrame.mapPartitions(rowsToAvro(_, schema)).rdd.saveAsHadoopFile(location,
